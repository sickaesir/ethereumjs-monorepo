...
export async function _insertAtNode(
  ...
  const _insert: {
    ...
    BranchNode: async () => {
      ...
      if (childNode) {
        ...
        const newChild = await _insertAtNode(childNode, keyNibbles, value, debug)
    ...
    ExtensionNode: async () => {
      ...
      if (sharedNibbles.length === extensionNode.getPartialKey().length) {
        ...
        const newChild = await _insertAtNode(
          extensionNode.child,
  ...
  const newRoot = await _cleanupNode(preCleanup, debug)
